...
This skill generates artificial stone color difference Lab datasets from a reference CSV.

It is designed for the user's 人造石色差 workflow.

The skill reads a reference CSV from:

人造石色差/data/

and outputs generated Excel files to:
...
- input CSV:
  人造石色差/data/{reference_name}.csv

- output Excel:
  人造石色差/output/{reference_name}_generated_lab_output.xlsx

The user may also provide a full reference CSV path. If a full path is provided, use it.

## Required Workflow

1. Read the V3 rule file.
2. Confirm the reference CSV exists.
3. Run the existing Lab generation workflow according to V3.
4. Generate the Excel output.
5. Verify:
...
7. If successful, commit automatically.
...
- Do not delete 人造石色差/data files.
